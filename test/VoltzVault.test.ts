...
import { ERC20RootVault, ERC20Vault, IMarginEngine, IVAMM, UniV3Vault, VoltzVault } from "./types";
import {
    combineVaults,
...
import { UNIV3_VAULT_INTERFACE_ID, VOLTZ_VAULT_INTERFACE_ID } from "./library/Constants";
        ...
        describe("rebalances position", () => {
            ...
            it("tvl if fees accumulated", async () => {
                ...
                await withSigner(randomAddress(), async (s) => {
                    ...
                    await this.marginEngineContract.connect(s).updatePositionMargin(
                        s.address,
                        -60,
                        0,
                        BigNumber.from(10).pow(6).mul(1000)
                    ...
                    const swapParams = {
                        recipient: s.address,
                        amountSpecified: BigNumber.from(10).pow(6).mul(1000),
                        sqrtPriceLimitX96: BigNumber.from("2507794810551837817144115957739"),
                        tickLower: -60,
                        tickUpper: 0
                    };
                    await this.vammContract.connect(s).swap(swapParams);
                });
